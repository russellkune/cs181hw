...
#        print "len(most_likely) = %d  len(actual) = %d" % (n, len(actual))
	for j in range(n):
	    ...
	    if actual[j] != most_likely[j]: errors = errors + 1
        if actual[j] != most_likely[j]: errors_by_state[int(actual[j][2])-1, int(actual[j][0]) -1] = errors_by_state[int(actual[j][2])-1, int(actual[j][0]) -1] + 1
